...
public class CalculoConsumoServiceImpl implements CalculoConsumoService {
    ...
    public ConsumoPeriodoResponse obtenerConsumoPeriodo(UUID contratoId, int mes, int anio) {
        ContratoAgua contrato = contratoRepository.findById(contratoId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Contrato no encontrado"));

        // 1. Intentar buscar lectura real
        return lecturaRepository.findByPeriodo(contratoId, mes, anio)
    ...
    private ConsumoPeriodoResponse calcularConsumoReal(ContratoAgua contrato, LecturaAgua actual) {
        ...
        BigDecimal lecturaAnterior = lecturaRepository.findFirstByContratoIdAndFechaLecturaBeforeOrderByFechaLecturaDesc(
    ...
    private ConsumoPeriodoResponse estimarConsumo(ContratoAgua contrato, int mes, int anio) {
        ...
        List<LecturaAgua> historial = lecturaRepository.findTop3ByContratoIdAndFechaLecturaBeforeOrderByFechaLecturaDesc(
